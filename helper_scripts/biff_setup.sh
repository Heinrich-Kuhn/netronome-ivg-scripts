...
BIFF_dir=$(find /root/IVG -name "BIFF")

# Copy BIFF to DUT
scp -i ~/.ssh/netronome_key -r $BIFF_dir root@$RX_DUT_IP:/root
MY_IP=$(ssh -i ~/.ssh/netronome_key root@$RX_DUT_IP who | tail -1 | egrep -o '([0-9]{1,3}\.){3}[0-9]{1,3}')
echo "MY IP: $MY_IP"

# Launch Oscar on RX_DUT
ssh -i ~/.ssh/netronome_key root@$RX_DUT_IP sed -i 's@TargetConnection IP=".*" PORT=".*"@TargetConnection IP=\"'$MY_IP'\" PORT=\"50123\"@g' /root/BIFF/Board-Instrumentation-Framework/Oscar/OscarConfig.xml
ssh -i ~/.ssh/netronome_key root@$RX_DUT_IP python3 /root/IVG/BIFF/Board-Instrumentation-Framework/Oscar/Oscar.py &

#Edit Marvin
sed -i 's@<Network IP=".*" PORT=".*" @<Network IP=\"'$MY_IP'\" PORT=\"50123\"@g'  /root/BIFF/Board-Instrumentation-Framework/Marvin/build/libs/Appliaction.xml
...
ssh -i ~/.ssh/netronome_key root@$RX_DUT_IP python3 Minion.py -i pktgenCapture.xml
