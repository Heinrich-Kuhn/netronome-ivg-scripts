...
sed -i '/^GRUB_CMDLINE_LINUX=.*/c\GRUB_CMDLINE_LINUX="intel_iommu=on iommu=pt intremap=on intel_idle.max_cstate=0 processor.max_cstate=0 idle=mwait intel_pstate=disable nohz_full=$nfp_cpulist rcu_nocbs=$nfp_cpulist transparent_hugepage=never"' /etc/default/grub
